...
def update_NH3_moment0(region_name='L1688', file_extension='DR1_rebase3', threshold=0.0125, save_masked=False):
    ...
    for line_i in ['11','22']:
        ...
        file_rms='{0}_NH3_{2}_{1}_rms_QA.fits'.format(region_name,file_extension,line_i)
        file_rms_mom='{0}_NH3_{2}_{1}_mom0_sigma_QA.fits'.format(region_name,file_extension,line_i)
        file_temp='{0}_NH3_{2}_{1}_masked_temp.fits'.format(region_name,file_extension,line_i)
        # Load pyspeckit cube
...
def cubefit(region='NGC1333', blorder=1, vmin=5, vmax=15, do_plot=False, 
    ...
    OneOneIntegrated = '{0}_NH3_11_{1}_mom0.fits'.format(region,root)
    OneOneFile = '{0}_NH3_11_{1}.fits'.format(region,root)
    RMSFile = '{0}_NH3_11_{1}_rms.fits'.format(region,root)
    TwoTwoFile = '{0}_NH3_22_{1}.fits'.format(region,root)
    ThreeThreeFile = '{0}_NH3_33_{1}.fits'.format(region,root)
